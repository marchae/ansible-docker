...
class DockerManager(object):
    ...
    def get_differing_containers(self):
        ...
        deployed = self.get_deployed_containers()
        current = self.get_inspect_containers(deployed)
...
def started(manager, containers, count, name):
    ...
    if delta > 0:
        ...
        if name and containers.deployed:
            to_start = containers.deployed
        else:
            to_start = manager.create_containers(delta)

        manager.start_containers(to_start)
        containers.notice_changed(manager.get_inspect_containers(to_start))
